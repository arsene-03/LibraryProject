...
<body id="firstPage">
	...
	<section>
		...
		<div class="container">
			<article class="frame">
				<div class="frame_con">
				<div class="suggestPageImg"></div>
					<div class="frame_inner">
						<div class="boardPageTitle">도서관에 바라다</div>
						
						<div class="totalSearch">
						<div class="totalCntBox">total_${totalCnt}</div>
						...
						<div class="searchBox">
							<form action="LS" method="POST">
								...
								<input type="text" name="searchText" class="searchText" placeholder="게시물을 검색해 보세요">
						...
						<table class="suggestTable">
							<%	
								List<BoardVO> boardList_suggest = (List)request.getAttribute("boardList");
								for(BoardVO board:boardList_suggest) {
									SimpleDateFormat asdf = new SimpleDateFormat("yyyy.MM.dd");
									String date = asdf.format(board.getBoard_time());
							%>
							<tr>
								<td><a href="LS?command=imgBoardList_info&idx=<%=board.getBoard_idx()%>&type=suggest"><%=board.getBoard_title()%><span><%=date%></span></a></td>
							</tr>
							<%
								}
							%>
								...
								<c:if test="${totalCnt != null}">
									<c:choose>
										...
										<!-- 이전버튼 -->  <a href="#"><img alt="pre" src="image/icon/preBut.png"></a>
													...
													<c:if test="${section >1 && page==1}">
										<!-- 이전버튼 -->  <a href="LS?command=SelectBoardList&type=suggest&section=${section-1}&pageNum=${10}"><img alt="pre" src="image/icon/preBut.png"></a>
													</c:if>
													
								        <!-- 페이지 넘버 --> <a href="LS?command=SelectBoardList&type=suggest&section=${section}&pageNum=${page}"><SPAN class ="page_number">&nbsp;&nbsp;${(section-1)*10+page}&nbsp;&nbsp;</SPAN><c:if test="${page<10}"></c:if></a>
													
													<c:if test="${page == 10}">
										<!-- 다음버튼 -->	<a href="LS?command=SelectBoardList&type=suggest&section=${section+1}&pageNum=${1}"><img alt="next" src="image/icon/nextBut.png"></a>
													</c:if>
												</c:forEach>
											</c:if>
											<c:if test="${(section)*100 > totalCnt}">
					...
					<!-- 관리자 접속 시 게시물 등록 버튼 -->	
						<c:if test="${member.member_type == 'admin'}">
							<div class="insertBtn">
								<input type="button" value="게시물 등록" onclick="location.href='LS?command=insertBoard_form_action&type=suggest'">
							</div>
						</c:if>
