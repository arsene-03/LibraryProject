...
import com.library.db.DBManager_mariaDB;
...
/////////////////////////////////////////////////////////////////
	public List<MemberVO> selectAllMember() { //회원정보 전체 조회메서드
		...
		try {
			conn = DBManager_mariaDB.connectDB();
			...
			DBManager_mariaDB.Close(conn, stmt, rs);
		}
		return memberList;
	}
/////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
	public void insertMember(MemberVO mVo) { //회원 가입 메서드
		...
		try {
			...
			DBManager_mariaDB.Close(conn, psmt);
		}
	}
/////////////////////////////////////////////////////////////////
	public List<BoardVO> selectBoardList(String type) { //게시판 조회 메서드
		...
		try {
			...
			DBManager_mariaDB.Close(conn, psmt, rs);
		}
		return boardList;
	}
/////////////////////////////////////////////////////////////////
	public List<ReviewVO> selectReviewList(int board_idx) { //댓글 조회 메서드
		
		String sql = "SELECT * FROM review WHERE board_idx = ? ORDER BY review_idx";
		...
		try {
			conn = DBManager_mariaDB.connectDB();
			psmt = conn.prepareStatement(sql);
			psmt.setInt(1, board_idx);
			rs = psmt.executeQuery();
			
			while(rs.next()) {
				ReviewVO rVo = new ReviewVO();
				rVo.setReview_idx(rs.getInt("review_idx"));
				rVo.setBoard_idx(rs.getInt("board_idx"));
				rVo.setMember_id(rs.getString("member_id"));
				rVo.setReview_content(rs.getString("review_content"));
				rVo.setReview_time(rs.getTimestamp("review_time"));
...
/////////////////////////////////////////////////////////////////
	public boolean insertBoard(BoardVO bVo) { //게시물 등록 메서드
		...
		String sql = "INSERT INTO `board` (`board_title`, `member_id`, `member_pwd`, `board_content`, `board_time`, `board_type`, `board_count`, `board_picture`, `board_picture2`, `board_picture3`, `board_picture4`, `board_picture5`, `board_picture6`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
...
/////////////////////////////////////////////////////////////////	
	public void insertcomment(ReviewVO rVo) { //댓글 등록 메서드
		String strtime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(System.currentTimeMillis());
		Timestamp time = Timestamp.valueOf(strtime);
		...
		try {
			...
			psmt.setInt(1,rVo.getBoard_idx());
			psmt.setString(2,rVo.getMember_id());
			psmt.setString(3,rVo.getReview_content());
...
/////////////////////////////////////////////////////////////////
	public boolean deleteBoard(String idx) { //게시물 삭제 메서드
		...
		try {
			...
			DBManager_mariaDB.Close(conn, stmt);
		}
		return true;
	}
/////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////
	public List<BoardVO> selectTargetBoard_img(int section, int pageNum, String type) { //9개씩 페이징 하는 메서드
		...
		String sql = 
				...
				/* ORACLE DB 쿼리문
				 * "SELECT * FROM (SELECT ROWNUM as nic, board_idx, board_title, member_id, member_pwd, board_content, board_time, board_type, board_count, board_picture, board_picture2, board_picture3, board_picture4, board_picture5, board_picture6 FROM"
					...
					"(SELECT @ROWNUM:=@ROWNUM+1 as ROWNUM, board_idx, board_title, member_id, member_pwd, board_content, board_time, board_type, "+ 
						"board_count, board_picture, board_picture2, board_picture3, board_picture4, board_picture5, board_picture6 "+
							"FROM (SELECT * FROM `board` WHERE board_type = ? ORDER BY board_idx)a)b "+
								"WHERE (SELECT @ROWNUM:=0)=0 BETWEEN (?-1)*90+(?-1)*9+1 AND (?-1)*90+?*9";
...
/////////////////////////////////////////////////////////////////
	public void deleteReview(int review_idx) { //댓글 삭제 메서드
		String sql = "DELETE FROM review WHERE review_idx="+review_idx;
...
/////////////////////////////////////////////////////////////////
	public Member_logVO selectMemberLog() { //회원 로그 관리
		String sql = "SELECt * FROM member_log";

		Connection conn = null;
		Statement stmt = null;
		ResultSet rs = null;
		Member_logVO mVoL = new Member_logVO();
		try {
			conn = DBManager_mariaDB.connectDB();
			stmt = conn.createStatement();
			rs = stmt.executeQuery(sql);
			...
			while(rs.next()) {
				mVoL.setMember_id(rs.getString("member_id"));
				mVoL.setMember_pwd(rs.getString("member_pwd"));
				mVoL.setMember_name(rs.getString("member_name"));
				mVoL.setMember_jumin(rs.getString("member_jumin"));
				mVoL.setMember_address(rs.getString("member_address"));
				mVoL.setMember_email(rs.getString("member_email"));
				mVoL.setMember_tel(rs.getString("member_tel"));
				mVoL.setMember_date(rs.getDate("member_date"));
				mVoL.setMember_type(rs.getString("member_type"));
				mVoL.setMember_point(rs.getInt("member_point"));
				mVoL.setModType(rs.getString("modType"));
