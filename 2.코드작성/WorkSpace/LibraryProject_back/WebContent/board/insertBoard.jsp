...
<body>
	...
	<table border="1">
		<tr>
			<th>제목</th>
			<td><input type="text" name="board_title" required="required" value="${board.getBoard_title()}"></td>
		</tr>
		<tr>	
			<th>게시판 유형</th>
			<td>
			...
			<%
				if(board==null){
					...
					if(board.getBoard_picture()!=null) {
						out.println("<td>"+board.getBoard_picture()+"<a href='LS?command=imgDelete_action&idx="+board.getBoard_idx()+"&picture="+board.getBoard_picture()+"'>삭제</a></td>");	
			...
			<%
				if(board==null){
					...
					if(board.getBoard_picture2()!=null) {
						out.println("<td>"+board.getBoard_picture2()+"<a href='LS?command=imgDelete_action&idx="+board.getBoard_idx()+"&picture2="+board.getBoard_picture2()+"'>삭제</a></td>");	
			...
			<%
				if(board==null){
					...
					if(board.getBoard_picture3()!=null) {
						out.println("<td>"+board.getBoard_picture3()+"<a href='LS?command=imgDelete_action&idx="+board.getBoard_idx()+"&picture3="+board.getBoard_picture3()+"'>삭제</a></td>");	
			...
			<%
				if(board==null){
					...
					if(board.getBoard_picture4()!=null) {
						out.println("<td>"+board.getBoard_picture4()+"<a href='LS?command=imgDelete_action&idx="+board.getBoard_idx()+"&picture4="+board.getBoard_picture4()+"'>삭제</a></td>");	
			...
			<%
				if(board==null){
					...
					if(board.getBoard_picture5()!=null) {
						out.println("<td>"+board.getBoard_picture5()+"<a href='LS?command=imgDelete_action&idx="+board.getBoard_idx()+"&picture5="+board.getBoard_picture5()+"'>삭제</a></td>");	
			...
			<%
				if(board==null){
					...
					if(board.getBoard_picture6()!=null) {
						out.println("<td>"+board.getBoard_picture6()+"<a href='LS?command=imgDelete_action&idx="+board.getBoard_idx()+"&picture6="+board.getBoard_picture6()+"'>삭제</a></td>");
		...
		<tr>
			<th>내용</th>
				<%if(board==null) {%>
				<td><div id="imgPlusText" ><textarea id="textbox" rows="10" cols="30"></textarea></div></td>
				
				<%}else{
					if(board.getBoard_content()!=null){%>
						<td><div id="imgPlusText" >${board.getBoard_content()}</div></td>		
					<%}else{%>
						<td><div id="imgPlusText" ><textarea id="textbox" rows="10" cols="30"></textarea></div></td>
					<%}
				}%>
		...
		<tr>
			<td colspan="2">*드래그해서 이미지를 글에 넣을 수 있습니다.<br>
					&nbsp이미지를 더블클릭하면 삭제할 수 있습니다.<br>
					&nbsp이미지가 불러와 질때가지 기다려주세요.*<br>					
			</td>
		</tr>
	</table>
	<div style="display: none">
		...
		if(board!=null){
			...
			out.println("<input type='submit' value='수정' onclick='return writeImgPlusText_hidden()'>");
		} else{
			out.println("<input type='submit' value='등록' onclick='return writeImgPlusText_hidden()'>");
		}
		%>

		<input type="button" value="돌아가기" onclick="location.href='LS?command=imgBoardList'">
	</form>
<!------스크립트------------------------------------------------------------------------------------------------------>
	<script>
		...
		switch(type) {
			case 'convenient': 
				document.getElementById("convenient").setAttribute("selected","selected");
				break;
			case 'notice':
				document.getElementById("notice").setAttribute("selected","selected");
				break;
			case 'event':
				document.getElementById("event").setAttribute("selected","selected");
			case 'suggest':
				document.getElementById("suggest").setAttribute("selected","selected");
		}
		/////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////// ajax 파일업로딩
			...
			req.onreadystatechange = function() {
				if((req.readyState==4)&&(req.status==200)){
					...
					setTimeout(function() { // 이미지 불러오는 시간때문에 지연시킴
						imgTag.setAttribute('src', 'image/'+req.responseText);
					} ,3000);
	...
	function writeImgPlusText_hidden() {
			document.getElementById("textbox").innerHTML = document.getElementById("textbox").value
